...
def solve(request, pr_id):
    ...
    first = test_cr[0]
    second = test_cr[1]
    return render(request, "solve.html", {"data": dat, "first": first, "second": second})
...
def report1(request):
    ...
    cur = base.cursor()
    cur.execute("Select gr_name, count(Distinct student_id) AS 'Количество студентов', count(Distinct pr_id) as 'Количество заданий' from (mainapp_problem A inner join mainapp_group B on A.pr_group_id = B.gr_id) С inner join mainapp_group_gr_students D on gr_id = group_id Group by gr_name Order By count(Distinct pr_id) DESC ")
    res = cur.fetchall()
    base.close()
...
def report2(request):
    ...
    base = sqlite3.connect("db.sqlite3")
    cur = base.cursor()
    cur.execute('Select pe_name,  count(CASE WHEN so_res=0 THEN 1 ELSE NULL END),  count(CASE WHEN so_res=5 THEN 1 ELSE NULL END), count(CASE WHEN so_res=10 THEN 1 ELSE NULL END), st_rate from mainapp_solving inner join mainapp_student on st_id=so_student_id inner join mainapp_person on st_profile_id=pe_id inner join mainapp_problem on so_problem_id=pr_id inner join mainapp_group on pr_group_id=gr_id Group by pe_name, st_rate')
    res = cur.fetchall()
    print(res)
    base.close()
    c = canvas.Canvas(resp)
